...
impl System{
    ...
    pub fn configure_lfclk_source(&self, clk: Clocks)->Result<(), Error>{
        self.wdt_unlock();
        match clk{
            Clocks::Ilo => {self.srss.clk_select.modify(|_,w| w.lfclk_sel().ilo()); self.wdt_lock(); Ok(())},
            Clocks::Wco => {self.srss.clk_select.modify(|_,w| w.lfclk_sel().wco()); self.wdt_lock(); Ok(())},
            Clocks::Pilo =>{self.srss.clk_select.modify(|_,w| w.lfclk_sel().pilo());  self.wdt_lock(); Ok(())},
            _ => {self.wdt_lock(); Err(Error::UnknownLfClkSource)}
        }
    }
    ...
    pub fn stop_lfclk(&self)->(){
        self.wdt_unlock();
        self.srss.clk_ilo_config.modify(|_, w| w.enable().clear_bit());
        self.wdt_lock();
    }
    ...
    pub fn configure_path_source(&self, path_num: ClockPath, path_source: PathSource) -> (){
            match path_source{
                PathSource::Imo => self.srss.clk_path_select[path_num as usize].modify(|_,w| w.path_mux().imo()),
                PathSource::ExtClk => self.srss.clk_path_select[path_num as usize].modify(|_,w| w.path_mux().extclk()),
                PathSource::Eco => self.srss.clk_path_select[path_num as usize].modify(|_,w| w.path_mux().eco()),
                PathSource::AltHf => self.srss.clk_path_select[path_num as usize].modify(|_,w| w.path_mux().althf()),
                PathSource::DsiMux => self.srss.clk_path_select[path_num as usize].modify(|_,w| w.path_mux().dsi_mux()),
            }
    }
    ...
    #[inline(always)]
    pub fn configure_dsi_mux_source(&self, dsi_select: SelectChannelNumber, dsi_mux_select:DsiMux) -> (){
            match dsi_mux_select{
                DsiMux::DsiOut0 => self.srss.clk_dsi_select[dsi_select as usize].modify(|_,w| w.dsi_mux().dsi_out0()),
                DsiMux::DsiOut1 => self.srss.clk_dsi_select[dsi_select as usize].modify(|_,w| w.dsi_mux().dsi_out1()),
                DsiMux::Ilo => self.srss.clk_dsi_select[dsi_select as usize].modify(|_,w| w.dsi_mux().ilo()),
                DsiMux::Wco => self.srss.clk_dsi_select[dsi_select as usize].modify(|_,w| w.dsi_mux().wco()),
                DsiMux::Pilo =>self.srss.clk_dsi_select[dsi_select as usize].modify(|_,w| w.dsi_mux().pilo()),
            }
    }
    ...
    pub fn configure_root_clk_source(&self, hf_index: RootClocks, clk_path: ClockPath)->(){
        match clk_path{
            ClockPath::Path0 => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_mux().path0());}
            ClockPath::Path1 => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_mux().path1());}
            ClockPath::Path2 => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_mux().path2());}
            ClockPath::Path3  => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_mux().path3());}
            ClockPath::Path4 => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_mux().path4());}
        }
    }
    #[inline(always)]
    pub fn configure_root_clk_div(&self, hf_index: RootClocks, clk_div: Divider)->(){
        match clk_div{
            Divider::NoDiv => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_div().no_div());}
            Divider::Div2 => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_div().div_by_2());},
            Divider::Div4 => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_div().div_by_4());},
            Divider::Div8 => {self.srss.clk_root_select[hf_index as usize].modify(|_, w| w.root_div().div_by_8());},
        }
    }
    #[inline(always)]
    ...
    pub fn configure_fll(&self, fll_config: FllConfig)->(){
        ...
        self.srss.clk_fll_config.modify(|_,w| unsafe{w.fll_mult().bits(fll_config.fll_mult)});
        self.srss.clk_fll_config2.modify(|_,w| unsafe{w.fll_ref_div().bits(fll_config.ref_div)});
        match fll_config.cco_range{
            FllCco::Range0 => self.srss.clk_fll_config4.modify(|_,w| w.cco_range().range0()),
            FllCco::Range1 => self.srss.clk_fll_config4.modify(|_,w| w.cco_range().range1()),
            FllCco::Range2 => self.srss.clk_fll_config4.modify(|_,w| w.cco_range().range2()),
            FllCco::Range3 => self.srss.clk_fll_config4.modify(|_,w| w.cco_range().range3()),
            FllCco::Range4 => self.srss.clk_fll_config4.modify(|_,w| w.cco_range().range4()),
        }
        match fll_config.output_div_enable{
            FllOutputDiv::Enable => self.srss.clk_fll_config.modify(|_,w| w.fll_output_div().set_bit()),
            FllOutputDiv::Disable => self.srss.clk_fll_config.modify(|_,w| w.fll_output_div().clear_bit()),
        }
        self.srss.clk_fll_config2.modify(|_, w| unsafe{w.lock_tol().bits(fll_config.lock_tolerance)});
    ...
    pub fn start_fll(&self, mut timeout_us:u32) -> Result<(), Error>{
        if self.srss.clk_fll_config.read().fll_enable().bit_is_clear(){
            ...
            self.srss.clk_trim_cco_ctl.modify(|_,w| w.enable_cnt().set_bit()); //start the cco stabilisation counter.
            //To enable the FLL, first enable the CCO by writing CLK_FLL_CONFIG4.CCO_ENABLE=1 and 
            self.srss.clk_fll_config4.modify(|_,w| w.cco_enable().set_bit());
            ...
            self.srss.clk_fll_config3.modify(|_, w| w.bypass_sel().fll_ref());
            ...
            if timeout_us > 0 {
                self.srss.clk_fll_config.modify(|_,w| w.fll_enable().set_bit());
            }
            // CLK_FLL_STATUS.LOCKED==1.
            //wait for the system to lock
            
            while  self.srss.clk_fll_status.read().locked().bit_is_clear()
                && error == Error::NoError{
                    match timeout_us > 0{
                        true => {timeout_us -= 1;
                                 delay(100);},
            ...
            self.srss.clk_fll_config3.modify(|_, w| w.bypass_sel().fll_out());
    ...
    pub fn stop_fll(&self)->(){
        if self.srss.clk_fll_config.read().fll_enable().bit_is_set(){
            ...
            self.srss.clk_fll_config.modify(|_,w| w.fll_enable().clear_bit());
            delay(1000);
            self.srss.clk_fll_config4.modify(|_,w| w.cco_enable().clear_bit());
    ...
    pub(crate) fn configure_clock_pump(&self)->(){
        self.srss.clk_select.modify(|_,w| unsafe{w.pump_sel().bits(0x00)});
        self.srss.clk_select.modify(|_,w| w.pump_div().div_by_4());
    }
    #[inline(always)]
    pub(crate) fn start_clock_pump(&self)->(){
        self.srss.clk_select.modify(|_, w| w.pump_enable().set_bit());
    }
    ...
    pub(crate) fn configure_ilo_hibernate(&self)->(){
        self.wdt_unlock();
        self.srss.clk_ilo_config.modify(|_,w| w.ilo_backup().set_bit());
        self.wdt_lock();
    }
